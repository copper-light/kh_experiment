...
def train(share, seed, epoch, model, dataset, criterion, optimizer, scheduler, args, batch_size = 384):
    ...
    for step, (data, target) in enumerate(train_set):
        ...
        if args.train_type == "minibatch":
            ...
            if (args.k > 0 and step % args.k == 0) or (step+1 == num_step):
                ...
                if step > 0:
                    idx = 0
                    for param in model.parameters():
                        param.grad.data += ((param.grad.data - recv_buffer[idx])  * args.lr)  
        ...
        elif args.train_type =="async_step":
            if ((step+1 < num_step)):
                if (args.k > 0 and step % args.k == 0):
                    ...
                    if step > 0:
                        ...
                        for param in model.parameters():
                            #param.grad.data += ((param.grad.data - (recv_buffer[idx]))  * args.lr)  
                            param.grad.data += ( (param.grad.data - recv_buffer[idx]) * (1/args.k) )
                            data = param.grad.data.clone().detach()
                            dist.all_reduce(data, op=dist.ReduceOp.SUM, async_op=True)
                            send_buffer.append(data)
                            idx += 1
        ...
        elif args.train_type =="loss_min":
            cur_std_loss, all_losses = std_mean_loss(losses.avg)
            std_loss.update(float(cur_std_loss))

            if num_step != step + 1:
                if ((cur_std_loss < (pre_std_loss_2 * args.loss_std_gap_rate))):
                    communication_count += 1
                    ...
                    if count <=1 :
                        ...
                        if len(recv_buffer) > 0:
                            ...
                            for param in model.parameters():
                                data = param.grad.data.clone().detach()
                                h = dist.all_reduce(data, op=dist.ReduceOp.SUM, async_op=True)
                                handles.append(h)
                                #h.wait()
                                send_buffer.append(data)
                                param.grad.data += ((param.grad.data - recv_buffer[idx]) * ( 1 / count ))
                                idx += 1
                        else:
                            for param in model.parameters():
                                data = param.grad.data.clone().detach()
            ...
            recent_loss.append(float(cur_std_loss))
            if len(recent_loss) > (5):
                recent_loss = recent_loss[-(5):]

            pre_std_loss = torch.min(torch.Tensor(recent_loss))
            ...
            if num_step != step + 1:
                if ((cur_std_loss > (pre_std_loss * args.loss_std_gap_rate))):
                    communication_count += 1
            ...
            pre_std_loss = torch.max(torch.Tensor(recent_loss))
            #pre_std_loss, _ = torch.std_mean(torch.Tensor(recent_loss))
            
        optimizer.step()

        #count += 1

        message = 'step time {:.4f}, epoch {:3}, step {:3}/{:3}, loss {:.8f}, auc@1 {:3.2f}, auc@5 {:3.2f}, share {}, std_loss {}, pre_std_loss {}, {}'.format(
            time.time() - start_step_time, epoch+1, step+1, num_step, losses.avg, top1.avg, top5.avg, communication_count, std_loss.avg, pre_std_loss, count
        )
    ...
    with torch.no_grad():
        ...
        message = 'train time {:.4f}, epoch {:3}, step {:3}, loss {:.8f}, auc@1 {:3.2f}, auc@5 {:3.2f}, share {}, std_loss {}, pre_std_loss {}, {}'.format(
            train_time, epoch+1, num_step, losses, top1, top5, communication_count, std_loss.avg, pre_std_loss, str_losses
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch', type=int, default=1024) # 384
    ...
    parser.add_argument('--ip', default='192.168.1.0:7548')
    ...
    # args.name = 'async_step(k=8)'
    # args.train_type = "async_step"
    # args.k = 8
