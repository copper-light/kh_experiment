...
def init_process(rank, size, fn, backend='nccl'):
    ...
    os.environ['MASTER_ADDR'] = '172.18.0.6'
...
#    os.environ['NCCL_IB_DISABLE'] = '1'
    ...
    dist.init_process_group(init_method="tcp://172.18.0.6:234", backend=backend, world_size=size)
    logInfo("========= connect =========")
...
if __name__ == "__main__":
    ...
    p = mp.Process(target=init_process, args=(rank, size, average_gradients))
    p.start()
    p.join()

    #torch.multiprocessing.spawn(init_process, nprocs=1, args=(proc, rank, size, average_gradients,))
